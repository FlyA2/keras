...
class SharpnessAwareMinimization(Model):
  ...
  are split via setting the `mini_batch_split_num` argument.

  Args:
    model: `tf.keras.Model` instance. The inner model that does the
      forward-backward pass.
    rho: float, defaults to 0.05. The gradients scaling factor.
    mini_batch_split_num: int, defaults to None. The number of mini batches to
      split each data batch. If None, batches are not split into mini batches.
  ...
  def __init__(self, model, rho=0.05, mini_batch_split_num=None, name=None):
    super().__init__(name=name)
    self.model = model
    self.rho = rho
    self.mini_batch_split_num = mini_batch_split_num
  ...
  def train_step(self, data):
    ...
    if self.mini_batch_split_num is not None:
      x_split = tf.split(x, self.mini_batch_split_num)
      y_split = tf.split(y, self.mini_batch_split_num)
    else:
      x_split = [x]
      y_split = [y]
